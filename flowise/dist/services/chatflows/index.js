...
const importChatflows = async (newChatflows) => {
    try {
        const appServer = (0, getRunningExpressApp_1.getRunningExpressApp)();
        // step 1 - check whether file chatflows array is zero
        if (newChatflows.length == 0)
            throw new Error('No chatflows in this file.');
        ...
        const selectResponse = await appServer.AppDataSource.getRepository(ChatFlow_1.ChatFlow)
            .createQueryBuilder('cf')
            .select('cf.id')
            .where(`cf.id IN ${ids}`)
            .getMany();
        ...
        const insertResponse = await appServer.AppDataSource.getRepository(ChatFlow_1.ChatFlow).insert(prepChatflows);
